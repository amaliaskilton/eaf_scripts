#EAF_ChangeOrthography_SingleTier
#Amalia Skilton 2020-06-11
#This script makes user-directed changes in the orthography used in all tiers of a given type in an EAF file.
#The script supports changes to 2 tiers with different orthographies.

### REQUIRED USER INPUTS ###

#State the path to the input file
input_file <- "/Users/amaliaskilton/Desktop/EAF_Scripts/EAF_ChangeOrthography/test_eafs/tca_201908_child_child27_cci_video_ahs.eaf"

#State the information for the FIRST type to change

#Type Name
targettype1_name <- "tca"

#Type Stereotype
#Is this tier a time-alignable tier or a referring tier (such as symbolic association)?
#If referring, change this line to REF. If alignable, keep as ALIGNABLE
targettype1_stereotype <- "ALIGNABLE"

#State the path to a csv file with a key to the practical orthography this tier.
key_file1 <- "/Users/amaliaskilton/Desktop/EAF_Scripts/EAF_ChangeOrthography/orthographykey.csv"

#State the information for the SECOND type to change. If none, comment these lines out

#Type Name
targettype2_name <- "tns"

#Type Stereotype
#Is this tier a time-alignable tier or a referring tier (such as symbolic association)?
#If alignable change this line to ALIGNABLE. If referring, keep as REF
targettype2_stereotype <- "REF"

#State the path to a csv file with a key to the practical orthography for type 2
...
#Do the type 1 tiers also contain some content in the type 2 orthography?
...
#Are we using a second orthography on tier type 1?
...
#Code for type 1 replacement
typepath1 <- paste(".//TIER[@LINGUISTIC_TYPE_REF='",targettype1_name,"']//ANNOTATION//",targettype1_stereotype,"_ANNOTATION",sep="")
typenode1 <- xml_find_all(eaf, typepath1)
for (i in 1:length(typenode1)) (
  xml_text(typenode1[[i]]) <- keysub_master(xml_text(typenode1[[i]]))
)

#Code for type 2 replacement
typepath2 <- paste(".//TIER[@LINGUISTIC_TYPE_REF='",targettype2_name,"']//ANNOTATION//",targettype2_stereotype,"_ANNOTATION",sep="")
typenode2 <- xml_find_all(eaf, typepath2)
for (i in 1:length(typenode2)) (
  xml_text(typenode2[[i]]) <- keysub(xml_text(typenode2[[i]]),key2)
)
